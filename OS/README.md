...
- 用户空间中的代码被限制了只能使用一个局部的内存空间，我们说这些程序在**用户态**（User Mode） 执行。

- 内核空间中的代码可以访问所有内存，我们称这些程序在**内核态**（Kernal Mode） 执行
...
1. 用户线程
   1. 优势：
    ...
    2. 缺点：
       1. 进行 I/O 的时候，无法利用到内核的优势，需要频繁进行用户态到内核态的切换
       2. 一个进程的一个用户态线程阻塞（Block）了，操作系统无法及时发现和处理阻塞问题，它不会更换执行其他线程，从而造成资源浪费
2. 内核线程
   1. 优点
      1. 内核拥有较高权限，因此可以在多个 CPU 核心上执行内核线程
      2. 内核中的线程操作 I/O 不需要进行系统调用；一个内核线程阻塞了，可以立即让另一个执行
   2. 缺点
      1. 创建的时候需要系统调用，也就是切换到内核态。
      2. 由一个内核程序管理，不可能数量太多
      3. 切换的时候，也同样存在需要内核操作，需要切换内核态
